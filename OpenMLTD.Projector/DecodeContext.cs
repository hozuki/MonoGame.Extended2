...
using OpenMLTD.Projector.AudioDecoding;
using OpenMLTD.Projector.Extensions;
using OpenMLTD.Projector.VideoDecoding;
...
namespace OpenMLTD.Projector {
    ...
    internal sealed unsafe class DecodeContext : DisposableBase {
        ...
        /// The reason is that subscribers (i.e. <see cref="Video.decodeContext_Ended"/>) may raise their events synchrously.
        /// The call flow can go to <see cref="VideoPlayer.video_Ended"/>, where <see cref="VideoPlayer.Stop"/> is called.
        /// Inside <see cref="VideoPlayer.Stop"/>, it calls <see cref="Thread.Join()"/> on <see cref="VideoPlayer.DecodingThread.SystemThread"/>,
        ...
        /// <summary>
        /// The <see cref="OpenMLTD.Projector.VideoDecoding.VideoDecodingContext"/> associated with this <see cref="DecodeContext"/>.
        ...
        /// <summary>
        /// The <see cref="OpenMLTD.Projector.AudioDecoding.AudioDecodingContext"/> associated with this <see cref="DecodeContext"/>.
        ...
        /// <summary>
        /// Reads audio packets, decode them into audio frames and send the data to an <see cref="AudioPlayer"/>, until presentation time of the frame decoded is after specified time.
        /// </summary>
        /// <param name="audioPlayer">The <see cref="AudioPlayer"/> to play audio data.</param>
        /// <param name="presentationTime">Current playback time, in seconds.</param>
        internal void HandleAudioPackets([NotNull] AudioPlayer audioPlayer, double presentationTime) {
            ...
            // Acquires a new AudioBufferUser.
            using (var user = audioPlayer.UseNewBuffer()) {
                using (var memoryStream = new MemoryStream()) {
...

                    {
                        ...
                        if (framePresentationTime > presentationTime) {
                    ...
                    if (dataPushed) {
                        ...
                        audioPlayer.ActivateContext();

                        var buffer = user.GetBuffer();
                        var audioData = memoryStream.ToArray();

                        // ... send the data to OpenAL's audio buffer...
                        buffer.LoadStereo16Bit(audioData, FFmpegHelper.RequiredSampleRate);

                        // ... and play it.
                        if (audioPlayer.SourceState == AudioState.Stopped) {
                            audioPlayer.Play();
                        }
                    }
                }
            ...
            // Finally, let AudioPlayer to update its buffers.
            audioPlayer.UpdateBuffers();
        }
        ...
        /// See <see cref="ReadVideoUntilPlaybackIsAfter"/> for its usage.
        /// </summary>
